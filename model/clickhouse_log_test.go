...
func TestClickHouseContextRequestLogCreateTableSQL(t *testing.T) {
	...
	assert.Contains(t, withoutTTL, "rule_id Int32 DEFAULT 0")
	assert.Contains(t, withoutTTL, "decision_source String DEFAULT ''")
	assert.Contains(t, withoutTTL, "request_body_truncated UInt8 DEFAULT 0")
	assert.Contains(t, withoutTTL, "response_body_truncated UInt8 DEFAULT 0")
	assert.Contains(t, withoutTTL, "ENGINE = MergeTree()")
	assert.Contains(t, withoutTTL, "ORDER BY (created_at, request_id)")
	assert.NotContains(t, withoutTTL, "TTL ")
