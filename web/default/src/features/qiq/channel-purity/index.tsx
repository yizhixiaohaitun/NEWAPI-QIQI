...
function ResultRow({ result, onOpen }: { result: TargetResult; onOpen: () => void }) {
  ...
  return (
    <TableRow>
      ...
      <TableCell className='max-w-72 font-mono text-xs'><span className='block truncate' title={`${result.baseline_model} → ${result.target_model}`}>{result.baseline_model} → {result.target_model}</span></TableCell>
...
function ResultCard({ result, onOpen }: { result: TargetResult; onOpen: () => void }) {
  ...
  return <Card className='gap-3 py-4'>
    <CardContent className='space-y-3 px-4'>
      <div className='flex items-start justify-between gap-2'><div><p className='font-medium'>{result.target_channel_name}</p><p className='text-muted-foreground max-w-[75vw] truncate text-xs' title={`${result.baseline_model} → ${result.target_model}`}>{result.baseline_model} → {result.target_model} · {t('Baseline')}: {result.baseline_channel_name}</p></div><StatusBadge status={result.status} /></div>
      <div className='grid grid-cols-3 gap-2'><button type='button' className='focus-visible:ring-ring cursor-pointer rounded text-left focus-visible:outline-none focus-visible:ring-2' onClick={onOpen} aria-label={t('View field and structure similarity details')}><Metric label={t('Field / structure')} value={result.field_similarity.value} /></button><Metric label={t('Token range')} value={result.token_similarity.value} /><Metric label={t('Confidence')} value={result.confidence} /></div>
...
function ResultDetail({ result, loading, error, running, runStatus, onRun, onRetry, onClose }: { result: TargetResult | null; loading: boolean; error?: string; running: boolean; runStatus?: string; onRun: () => void; onRetry: () => void; onClose: () => void }) {
  const { t } = useTranslation()
