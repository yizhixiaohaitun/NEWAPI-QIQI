...
type ResultFilter = 'all' | 'attention' | 'collecting' | 'healthy'

const attentionStatuses = new Set<DetectorStatus>([
  'ALERT',
  'SUSPECT',
  'DETECTOR_ERROR',
...
function SummaryCard({ icon, label, value, note, tone = 'default' }: { icon: React.ReactNode; label: string; value: number; note: string; tone?: 'default' | 'attention' | 'collecting' | 'healthy' }) {
...
function TargetResultGroup({ entries, onOpen }: { entries: ResultEntry[]; onOpen: (result: TargetResult) => void }) {
  ...
  const attentionCount = entries.filter(({ result }) => resultCategory(result.status) === 'attention').length
...
function ResultDetail({ result, minimumSamples, loading, error, running, runStatus, onRun, onRetry, onClose }: { result: TargetResult | null; minimumSamples: number; loading: boolean; error?: string; running: boolean; runStatus?: string; onRun: () => void; onRetry: () => void; onClose: () => void }) {
  ...
  return <Dialog open={Boolean(result) || loading || Boolean(error)} onOpenChange={(open) => { if (!open) onClose() }}><DialogContent className='max-h-[90vh] overflow-y-auto sm:max-w-4xl'>{result ? <>
    <DialogHeader><DialogTitle className='break-all'>{result.target_channel_name} · {result.baseline_model} → {result.target_model}</DialogTitle><DialogDescription>{t('Independent comparison against baseline {{baseline}}', { baseline: result.baseline_channel_name })}</DialogDescription></DialogHeader>
    <div className='flex flex-wrap items-center gap-2'><Button size='sm' variant='outline' disabled={running} onClick={onRun}><Play />{running ? t('Starting manual detection…') : t('Run manual detection')}</Button>{runStatus ? <span role='status' aria-live='polite' className='text-muted-foreground text-sm'>{runStatus}</span> : null}</div>
    <div className={`rounded-lg border p-4 ${category === 'attention' ? 'border-destructive/35 bg-destructive/5' : category === 'collecting' ? 'border-amber-500/35 bg-amber-500/5' : 'border-emerald-500/35 bg-emerald-500/5'}`}>
    ...
    <div className='grid gap-3 sm:grid-cols-3'>
      <ScorePanel label={observingOnly ? t('Structure similarity (observed)') : t('Structure similarity')} value={percent(result.field_similarity.value)} note={observingOnly ? t('Not used for a conclusion yet') : t('Compared with the designated baseline')} />
      <ScorePanel label={observingOnly ? t('Token similarity (observed)') : t('Token similarity')} value={percent(result.token_similarity.value)} note={observingOnly ? t('Not used for a conclusion yet') : t('Compared with the designated baseline')} />
    ...
    <Tabs defaultValue='overview'>
      ...
      <TabsContent value='evidence' className='space-y-3 pt-2'>
        ...
        <div><h3 className='mb-2 font-medium'>{t('Original detector evidence')}</h3>{result.evidence.length ? <div className='space-y-2'>{result.evidence.map((item) => <div key={item.id} className='rounded-lg border p-3'><div className='flex justify-between gap-3'><Badge variant='outline'>{item.kind}</Badge><span className='text-muted-foreground text-xs'>{formatTime(item.occurred_at)}</span></div><p className='mt-2 text-sm'>{t(item.summary)}</p>{item.baseline_value !== undefined || item.target_value !== undefined ? <div className='bg-muted mt-2 grid gap-2 rounded p-2 font-mono text-xs sm:grid-cols-2'><span>{t('Baseline')}: {item.baseline_value ?? '—'}</span><span>{t('Target')}: {item.target_value ?? '—'}</span></div> : null}{item.request_id ? <p className='text-muted-foreground mt-2 text-xs'>Request ID: {item.request_id}</p> : null}</div>)}</div> : <p className='text-muted-foreground text-sm'>{t('No evidence has been recorded yet.')}</p>}</div>
      </TabsContent>
      <TabsContent value='history' className='pt-2'>
        <h3 className='mb-2 font-medium'>{t('Historical trend')}</h3>{result.trend.length ? <div className='grid auto-cols-[minmax(120px,1fr)] grid-flow-col gap-2 overflow-x-auto pb-2'>{result.trend.map((point, index) => <div key={`${point.at}-${index}`} className='min-w-30 rounded-lg border p-3'><div className={`mb-2 h-1 rounded ${point.status === 'ALERT' || point.status === 'DETECTOR_ERROR' ? 'bg-destructive' : point.status === 'SUSPECT' || collectingStatuses.has(point.status) ? 'bg-amber-500' : 'bg-emerald-500'}`} /><p className='text-xs font-medium'>{t(`Purity detector status: ${point.status}`)}</p><p className='text-muted-foreground mt-2 text-[11px]'>{formatTime(point.at)}</p><div className='mt-2 grid grid-cols-2 gap-2 text-xs'><span>{t('Structure')}<b className='block tabular-nums'>{percent(point.field_similarity)}</b></span><span>{t('Token')}<b className='block tabular-nums'>{percent(point.token_similarity)}</b></span></div></div>)}</div> : <p className='text-muted-foreground text-sm'>{t('Trend is unavailable until multiple detection windows are recorded.')}</p>}
      </TabsContent>
...
export function ChannelPurity() {
  ...
  const clearHistoryMutation = useMutation({ mutationFn: clearPurityGroupHistory, onSuccess: async () => { toast.success(t('Detection history cleared')); closeDetail(); await refresh() }, onError: (error) => toast.error(errorMessage(error) || t('Failed to clear detection history')) })
  const toggleMutation = useMutation({ mutationFn: async (group: PurityGroup) => updatePurityGroup(group.id, { name: group.name, enabled: !group.enabled, channel_ids: group.channel_ids, baseline_channel_id: group.baseline_channel_id, interval_minutes: group.interval_minutes, random_pairing_enabled: group.random_pairing_enabled, model_comparisons: group.model_comparisons.map((comparison) => ({ ...comparison })), sampling: { ...group.sampling } }), onSuccess: async () => { toast.success(t('Group status updated')); await refresh() }, onError: (error) => toast.error(errorMessage(error) || t('Failed to update group status')) })
  ...
  const counts = useMemo(() => ({
    attention: results.filter(({ result }) => resultCategory(result.status) === 'attention').length,
  ...
  return <SectionPageLayout>
    ...
    <SectionPageLayout.Content><div className='space-y-5'>
      ...
      <div className='grid gap-3 sm:grid-cols-2 xl:grid-cols-4'>
        ...
        <SummaryCard icon={<ShieldAlert className='text-destructive size-4' />} label={t('Need attention')} value={counts.attention} note={t('Errors, alerts, suspects, or unavailable baselines')} tone='attention' />
      ...
      <Card>
        <CardHeader className='border-b'><div className='flex flex-wrap items-start justify-between gap-3'><div><CardTitle>{t('Detection results')}</CardTitle><p className='text-muted-foreground mt-1 text-sm'>{t('Results needing attention are shown first. Similarity percentages are not treated as a conclusion while samples are still accumulating.')}</p></div><div className='flex flex-wrap gap-1 rounded-lg bg-muted p-1'>{([
          ...
          ['attention', t('Need attention {{count}}', { count: counts.attention })],
