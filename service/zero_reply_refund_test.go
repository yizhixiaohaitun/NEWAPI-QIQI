...
func TestZeroReplyAutoRefundEnabledHit(t *testing.T) {
	...
	refunded := MaybeAutoRefundZeroReplyQuota(ctx, relayInfo, "gpt-test", 120, 0, 50)
...
func TestZeroReplyAutoRefundConditionNotMet(t *testing.T) {
	...
	// completion > 0 → 不退
	assert.False(t, MaybeAutoRefundZeroReplyQuota(ctx, relayInfo, "gpt-test", 120, 5, 50))
	// prompt = 0 → 不退
	assert.False(t, MaybeAutoRefundZeroReplyQuota(ctx, relayInfo, "gpt-test", 0, 0, 50))
	// quota = 0（本次没扣费）→ 不退
	assert.False(t, MaybeAutoRefundZeroReplyQuota(ctx, relayInfo, "gpt-test", 120, 0, 0))
...
func TestZeroReplyAutoRefundNoDoubleRefund(t *testing.T) {
	...
	// 第一次结算出口：退款成功
	assert.True(t, MaybeAutoRefundZeroReplyQuota(ctx, relayInfo, "gpt-test", 120, 0, 50))
	// 同一请求第二次结算出口：不再退款
	assert.False(t, MaybeAutoRefundZeroReplyQuota(ctx, relayInfo, "gpt-test", 120, 0, 50))
